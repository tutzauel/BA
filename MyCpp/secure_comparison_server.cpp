...
namespace SecureFaceRecognitionUtils {
	...
	*/
	Paillier::Ciphertext SecureComparisonServer::Compare (const Paillier::Ciphertext &a, const Paillier::Ciphertext &b) {
		...
		std::cout << "secure_comparison_server.cpp" << std::endl;		
		/// Compute @f$ [z] = [2^l + a - b] = [2^l] [a] [b]^{-1} @f$

		std::cout << "Server berechnet [z] = [2^l]*[a]*[b]^{-1}" << std::endl;
		...
		std::cout << "Server tarnt z und berechnet [d] =  [z]*[r]" << std::endl;
		...
		*/
		...
		std::cout << "Server schickt getarntes lambda an Client" << std::endl;
